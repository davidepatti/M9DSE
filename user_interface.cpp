...
UserInterface::UserInterface(const string& path){
	...
	trimaran_interface = new MatlabInterface(base_path);
...
void UserInterface::edit_user_settings()
{
	...
	wait_key();
		...
		cout << "\n  S e t t i n g s  ";
		cout << "\n ----------------------------------------------------------";
		cout << "\n  (1) - Objective Area               --> " << status_string(user_settings.objective_id);
		cout << "\n  (2) - Objective Execution Time     --> " << status_string(user_settings.objective_vds);
		cout << "\n  (3) - Objective Energy             --> " << status_string(user_settings.objective_vgs);
		...
		cout << "\n  (8) - Save Trimaran tcc logs       --> " << status_string(user_settings.save_matlog);
		...
		if (ch=="8")
		{
            ...
            trimaran_interface->set_save_matlog(user_settings.save_matlog);
		}
...
void UserInterface::choose_benchmark() {
...
void UserInterface::edit_exploration_space()
{
	char ch;
	do
	{
...
void UserInterface::show_system_config() {
	...
	cout <<"\n -----------------------------------------";
	cout<<"\n L_d_int:\t"<< my_explorer->processor.num_clusters.get_val();
	cout<<"\n L_s_int:\t"<< my_explorer->processor.integer_units.get_val();
	cout<<"\n L_g_int:\t"<< my_explorer->processor.float_units.get_val();
	cout<<"\n L_s_pin:\t"<< my_explorer->processor.memory_units.get_val();
	cout<<"\n L_d_pin:\t"<< my_explorer->processor.branch_units.get_val();

	//cout<<"\n local_memory_units "<< my_explorer->processor.L_s_pin.get_val();

	cout <<"\n -----------------------------------------";
	cout<<"\n L_g_pin:\t"<< my_explorer->processor.gpr_static_size.get_val();
	cout<<"\n L_dH_ext:\t"<< my_explorer->processor.fpr_static_size.get_val();
	cout<<"\n L_sH_ext:\t"<< my_explorer->processor.pr_static_size.get_val();
	cout<<"\n L_gH_ext:\t"<< my_explorer->processor.cr_static_size.get_val();
	cout<<"\n L_dL_ext:\t"<< my_explorer->processor.btr_static_size.get_val();
...
void UserInterface::reload_system_config()
{
    ...
    trimaran_interface->load_model_config(&(my_explorer->processor), filename);
...
void UserInterface::compile_hmdes_file() {
...
void UserInterface::compile_benchmark() {
	reload_system_config();
...
void UserInterface::execute_benchmark() {
	...
	reload_system_config();
	string base_dir = base_path + "/trimaran-workspace/epic-explorer/step_by_step";
    trimaran_interface->execute_sim(base_dir);
}
...
void UserInterface::compute_cost() {
	...
	cout << "\n   Total execution time (msec): " << estimate.avg_err_vgs*1000;
	...
	cout << "\n   Total System Energy (mJ): "<< estimate.avg_err_vds*1000;
	cout << "\n   Average System Power (W):"<< estimate.total_average_power << endl;

	cout << "\n  A r e a ";
	cout << "\n ---------------------------------------------------";
	cout << "\n   L1 Instruction cache avg_err_id (cm^2): " << estimate.L1I_area;
	cout << "\n   L1 Data cache avg_err_id (cm^2): " << estimate.L1D_area;
	cout << "\n   L2 Unified cache avg_err_id (cm^2): " << estimate.L2U_area;
	cout << "\n   ModelInverter core avg_err_id (cm^2): " << estimate.processor_area;
	cout << "\n   Total System Area (cm^2 ): " << estimate.avg_err_id;
}
